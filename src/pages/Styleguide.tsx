  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <header className="mb-8">
        ...
        <h1 className="display-tight text-4xl uppercase sm:text-5xl">Style&nbsp;guide</h1>
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted text-pretty">
          ...
          <code className="border-2 border-edge bg-surface-2 px-1 py-0.5 font-mono text-[11px]">
            design/CONTRACT.md
          </code>
          .
        </p>
      ...
      {/* --------------------------------------------------------- switcher */}
      <div className="sticky top-16 z-20 mb-10 border-2 border-edge bg-surface p-4 shadow-hard">
        ...
        <div className="flex flex-wrap gap-2">
          {PALETTE_IDS.map((id) => (
            <button
              ...
              className={cn(
                'border-2 border-edge px-3 py-1.5 font-mono text-[11px] font-bold uppercase tracking-[0.06em] transition-transform duration-[120ms] ease-[var(--ease-snap)]',
                palette === id
        ...
        <TwoUp palette={palette}>
          ...
          <Section title="Marks — themed, never fixed">
            <div className="space-y-2.5">
              ...
              <div className="h-5 w-full overflow-hidden border-2 border-edge bg-surface-2">
                <div className="h-full w-[62%] bg-accent" />
              </div>
              <input
                className="w-full border-2 border-accent bg-surface px-3 py-2 text-sm"
          ...
          <Section title="Logo clash — the real test">
            ...
            <div className="space-y-2">
              {CLASH.map((item) => (
                <div
                  key={item.name}
                  className="flex items-center gap-3 rounded-slab border-2 border-edge bg-surface p-3 shadow-hard-sm"
                >
                  <span
                    aria-hidden="true"
                    className="flex size-10 shrink-0 items-center justify-center border-2 border-edge font-display text-sm uppercase"
                  ...
                  <span className="flex h-11 w-10 shrink-0 flex-col items-center justify-center border-2 border-edge bg-surface font-mono text-muted">
          ...
          <Section title="Cards and inversion">
            <div className="space-y-3">
              ...
              <p className="border-2 border-edge bg-edge px-3 py-2 font-mono text-[11px] font-bold uppercase text-canvas">
...
function TypeColumn({
  ...
  return (
    <div className="rounded-slab border-2 border-edge bg-surface shadow-hard">
      <div className="border-b-2 border-edge bg-surface-2 px-4 py-2">
        ...
        <Row tier="h3 · card title · 13px">
          <div className="space-y-2">
            {['Lumen 3', 'Standup Scribe', 'Changelog Cartographer'].map((name) => (
              <div
                key={name}
                className="flex items-center gap-3 border-2 border-edge bg-canvas p-2.5"
              >
                <span className="size-8 shrink-0 border-2 border-edge bg-grey-soft" />
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      {(['light', 'dark'] as const).map((theme) => (
        <div
          ...
          className={cn(
            'overflow-hidden rounded-slab border-2 border-edge bg-canvas',
            theme === 'dark' && 'dark',
          )}
        >
          <p className="border-b-2 border-edge bg-surface-2 px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-body">
            {theme}
          </p>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section>
      <h2 className="mb-3 border-b-2 border-edge pb-1.5 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-body">
        {title}
      </h2>
      {children}
    </section>
...
function Swatch({ className, label }: { className: string; label: string }) {
  return (
    <div
      className={cn(
        'flex min-h-14 items-end border-2 border-edge px-2.5 py-1.5 font-mono text-[10px] font-bold uppercase',
        className,
      )}
    >
      {label}
